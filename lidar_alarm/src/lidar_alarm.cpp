...
int laser_counter = 0; // counter for #times that ping_dist_in_front_<MIN_SAFE_DISTANCE
...
void laserCallback(const sensor_msgs::LaserScan& laser_scan) {
    if (ping_index_<0)  {
        ...
        ROS_INFO("anglemin = %d",angle_min_);
        ROS_INFO("angle increment = %d",angle_increment_);
        ROS_INFO("rangemin = %d",range_min_);
        ROS_INFO("rangemax = %d",range_max_);
        
    }

   laser_counter = 0;
   for(int i=160; i<=500; i++){//robot examines an array of lidar "pings"
