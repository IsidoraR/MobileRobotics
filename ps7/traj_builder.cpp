...
void TrajBuilder::build_braking_traj(geometry_msgs::PoseStamped start_pose,
    ...
    double x_brake = start_pose.pose.position.x;
    ...
    for(int i=0; i<vec_state; i++){
    	int diff_x = start_pose.pose.position.x - vec_of_states[i].pose.pose.position.x;
    	int diff_y = start_pose.pose.position.y - vec_of_states[i].pose.pose.position.y;
    	if(abs(diff_x)<0.1 && abs(diff_y)<0.1){
    		des_state.twist.twist = vec_of_states[i].twist.twist;
    		break;
    	}
    }
    brake_vel = des_state.twist.twist.linear.x;
    current_vel = brake_vel;
    brake_omega = des_state.twist.twist.angular.z;
    current_omega = brake_omega;

    while(current_vel>0 && current_omega>0){
    	brake_time = brake_time + dt_;
    	current_vel = brake_vel - (accel_max_ * brake_time);
    	des_state.twist.twist.linear.x = current_vel;
    	x_des = x_brake + (brake_vel * brake_time) - (0.5 * accel_max_ * brake_time * brake_time *cos(psi_des));
    	y_des = y_brake + (brake_vel * brake_time) - (0.5 * accel_max_ * brake_time * brake_time *cos(psi_des));
    	current_omega = brake_omega - alpha_max_ * brake_time;
