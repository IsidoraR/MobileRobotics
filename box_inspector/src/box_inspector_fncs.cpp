	...
	void BoxInspector::compute_shipment_poses_wrt_world(osrf_gear::Shipment shipment_wrt_box,
		...
		int nmodels=desired_models_wrt_world.size();
		ROS_WARN(" number of models = %d", nmodels);
		for(int i=0; i<nmodels; i++){
			ROS_INFO_STREAM("desired part poses w/rt world: " << desired_models_wrt_world[i] << endl);
	    
		}
 
		//ROS_INFO_STREAM("desired part poses w/rt world: " << desired_models_wrt_world << endl);
