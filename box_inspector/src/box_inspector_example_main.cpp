...
int main(int argc, char ** argv) {
  ...
  vector<osrf_gear::Model> observed_models_wrt_world;
  ...
  //vectors to hold the inspection  results:
   ...
   vector<osrf_gear::Model> orphan_models_wrt_world;
    ...
    int n_products = shipment_status.products.size();
    observed_models_wrt_world.clear();
    osrf_gear::Model model;
    for(int i = 0; i < n_products; i++){
	model.type = shipment_status.products[i].type;
	model.pose = shipment_status.products[i].pose;
	observed_models_wrt_world.push_back(model);
    }
    ...
    boxInspector.update_inspection(observed_models_wrt_world, desired_models_wrt_world, satisfied_models_wrt_world,
